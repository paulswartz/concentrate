defmodule Concentrate.Producer.GunTest do
  @moduledoc false
  use ExUnit.Case, async: false
  import Concentrate.Producer.Gun
  import Plug.Conn, only: [get_req_header: 2, put_resp_header: 3, send_resp: 3]

  doctest Concentrate.Producer.Gun
  ...
  #     state = %Concentrate.Producer.Gun.State{machine: machine}
  ...
  #     machine = StateMachine.init("url", parser: &[&1])
  #     state = %Concentrate.Producer.Gun.State{machine: machine, demand: 1}
  ...
  describe "bypass" do
    ...
    test "a fetch error is not fatal" do
      {:ok, pid} =
        start_supervised({Concentrate.Producer.Gun, {"http://nodomain.dne", parser: & &1}})
    ...
    defp start_producer(bypass, opts \\ []) do
      ...
      {:ok, _} = start_supervised({Concentrate.Producer.Gun, {url, opts}})
    end
