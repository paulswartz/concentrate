defmodule Concentrate.Producer.Hackney.PropertyTest do
  @moduledoc false
  ...
  property "returns all the bodies" do
    check all bodies <- list_of(bodies(), min_length: 1, max_length: 10),
      ...
      {:ok, producer} =
        start_supervised({Concentrate.Producer.Hackney, {url, parser: &parser/1, fetch_after: 1}})
  ...
  defp receive_count?(producer, demand, count) do
    ...
    :ok = stop_supervised(Concentrate.Producer.Hackney)
    passed?
  end
end
