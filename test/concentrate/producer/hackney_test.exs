defmodule Concentrate.Producer.HackneyTest do
  @moduledoc false
  use ExUnit.Case
  import Concentrate.Producer.Hackney
  alias Concentrate.Producer.Hackney.StateMachine
  ...
  describe "handle_info/2" do
    ...
    test "ignores unknown messages" do
      ...
      state = %Concentrate.Producer.Hackney.State{machine: machine}
    ...
    test "does not send more demand than requested" do
      machine = StateMachine.init("url", parser: &[&1])
      state = %Concentrate.Producer.Hackney.State{machine: machine, demand: 1}
  ...
  describe "bypass" do
    ...
    test "a fetch error is not fatal" do
      {:ok, pid} =
        start_supervised({Concentrate.Producer.Hackney, {"nodomain.dne", parser: & &1}})
    ...
    defp start_producer(bypass, opts \\ []) do
      ...
      {:ok, _} = start_supervised({Concentrate.Producer.Hackney, {url, opts}})
    end
