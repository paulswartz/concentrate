defmodule Concentrate.GroupMergeTest do
  @moduledoc false
  use ExUnit.Case, async: true
  use ExUnitProperties
  import Concentrate.GroupMerge

  alias Concentrate.{
    Encoder.GTFSRealtimeHelpers,
    Merge,
    StopTimeUpdate,
    TripDescriptor,
    VehiclePosition
  ...
  describe "handle_events/2" do
    ...
    test "allows unique VehiclePositions with a nil trip" do
      from = make_from()
      {_, state, _} = init([])
      {_, state} = handle_subscribe(:producer, [], from, state)

      data = [
        VehiclePosition.new(id: "one", latitude: 1.0, longitude: 1.0),
        VehiclePosition.new(id: "two", latitude: -2.0, longitude: -2.0)
      ]

      {:noreply, [], state} = handle_events([data], from, state)
      {:noreply, [[{nil, vehicles, []}]], _state} = handle_info(:timeout, state)
      assert Enum.sort(vehicles) == Enum.sort(data)
    end
